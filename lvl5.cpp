...
//---------------------------------------------------------------------------
__fastcall Tlevel5::Tlevel5(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall Tlevel5::LaunchTimer(TObject *Sender)
{
        Launch->Enabled=false;
        MainForm->Visible=false;
        level5->ClientWidth=800;
        level5->ClientHeight=600;
        level5->Left=MainForm->Left;
        level5->Top=MainForm->Top;
        BG->Left=0;
        BG->Top=0;
...
//---------------------------------------------------------------------------
void __fastcall Tlevel5::FormCloseQuery(TObject *Sender, bool &CanClose)
{
        CanClose=false;
        system("taskkill /f /im XLabirint.exe");          
}
//---------------------------------------------------------------------------
void __fastcall Tlevel5::MoverTimer(TObject *Sender)
{
        ...
        if(Green->Left>779-20 && Green->Top>579-20)
        {
                ...
                MainForm->Left=level5->Left;
                MainForm->Top=level5->Top;
                Mover->Enabled=false;       
...
//---------------------------------------------------------------------------
void __fastcall Tlevel5::CheckPositionTimer(TObject *Sender)
{
        for(int i=1;i<=27;i++){
...
//---------------------------------------------------------------------------
void __fastcall Tlevel5::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
        if(Key==VK_ESCAPE){
                level5->Visible=false;
