...
bool s19=false;
//---------------------------------------------------------------------------
__fastcall Tlevel6::Tlevel6(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall Tlevel6::LaunchTimer(TObject *Sender)
{
        Launch->Enabled=false;
        MainForm->Visible=false;
        level6->ClientWidth=800;
        level6->ClientHeight=600;
        level6->Left=MainForm->Left;
        level6->Top=MainForm->Top;
        BG->Left=0;
        BG->Top=0;
...
//---------------------------------------------------------------------------
void __fastcall Tlevel6::FormCloseQuery(TObject *Sender, bool &CanClose)
{
        CanClose=false;
        system("taskkill /f /im XLabirint.exe");         
}
//---------------------------------------------------------------------------
void __fastcall Tlevel6::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
        if(Key==VK_ESCAPE){
                level6->Visible=false;
                MainForm->Left=level6->Left;
                MainForm->Top=level6->Top;
                Mover->Enabled=false;
...
//---------------------------------------------------------------------------
void __fastcall Tlevel6::CheckPositionTimer(TObject *Sender)
{
        for(int i=1;i<=22;i++){
...
//---------------------------------------------------------------------------
void __fastcall Tlevel6::MoverTimer(TObject *Sender)
{
        if(Green->Top<0 && level6->Top>=0)level6->Top-=1;
        else if(Green->Top>600-Green->Height && level6->Top<Screen->Height-600)level6->Top+=1;
        else if(Green->Left<0 && level6->Left>=0)level6->Left-=1;
        else if(Green->Left>800-Green->Height && level6->Left<Screen->Width-800)level6->Left+=1;
        ...
        if(Red->Top<0 && level6->Top>=0)level6->Top-=1;
        else if(Red->Top>600-Red->Height && level6->Top<Screen->Height-600)level6->Top+=1;
        else if(Red->Left<0 && level6->Left>=0)level6->Left-=1;
        else if(Red->Left>800-Red->Height && level6->Left<Screen->Width-800)level6->Left+=1;
