...
//---------------------------------------------------------------------------
__fastcall Tlevel2::Tlevel2(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall Tlevel2::LaunchTimer(TObject *Sender)
{
        Launch->Enabled=false;
        MainForm->Visible=false;
        level2->ClientWidth=800;
        level2->ClientHeight=600;
        level2->Left=MainForm->Left;
        level2->Top=MainForm->Top;
        BG->Left=0;
        BG->Top=0;
...
//---------------------------------------------------------------------------
void __fastcall Tlevel2::FormCloseQuery(TObject *Sender, bool &CanClose)
{
        CanClose=false;
        system("taskkill /f /im XLabirint.exe");         
}
//---------------------------------------------------------------------------
void __fastcall Tlevel2::FormKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
        if(Key==VK_ESCAPE){
                level2->Visible=false;
                MainForm->Left=level2->Left;
                MainForm->Top=level2->Top;
                Mover->Enabled=false;
...
//---------------------------------------------------------------------------
void __fastcall Tlevel2::MoverTimer(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall Tlevel2::CheckPositionTimer(TObject *Sender)
{
        for(int i=1;i<=9;i++){
